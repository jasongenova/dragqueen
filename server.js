...
function doesCollide(rect1, rect2) {
  if (rect1.x && rect1.y && rect2.x && rect2.y) {
  	var x1 = rect1.x;
  	var y1 = rect1.y;
  	var h1 = rect1.height;
  	var w1 = rect1.width;
...
function getVector(width, height) {
    var vector = {
    ...
    while (!checked) {
      ...
      for (var user in clients) {
        if (doesCollide(clients[user], vector)) {
...
function randomize() {
  for (var user in clients) {
    var vector = getVector(PW, PH);
    clients[user].x = vector.x;
    clients[user].y = vector.y;
  }
}
        ...
        if (timer < 0) {
            ...
            setTimeout(function () {
              if (Object.keys(clients).length > 0) {
                ...
                showThings();
                startTimer(ROUND_LENGTH, socket);
              }
            }, ROUND_BREAK);
        }
    }, 1000);
